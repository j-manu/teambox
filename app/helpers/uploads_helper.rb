module UploadsHelper
  ...
  def docs_list_tag(doc_id)
    content_tag(:input,nil,{ :name => "google_docs[]",:type => 'hidden',:value => doc_id })
  end
  ...
  def google_doc_image(doc,size='48px')
    extensions = {:document => "doc", :spreadsheet => "xls", :presentation => "ppt"}
    extension = extensions[doc.doc_type.to_sym] || doc.doc_type
    if Upload::ICONS.include?(extension)
      image_tag("file_icons/#{size}/#{extension}.png", :class => "file_icon #{extension}")
    else
      image_tag("file_icons/#{size}/_blank.png")
  ...
  def list_google_docs_inline(google_docs)
    render :partial => 'google_docs/google_doc', :collection => google_docs, :as => :google_doc
  end
